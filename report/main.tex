...
This section derives the MaxSim analogue of the BOND bound, states the
pruning rules it supports, and proves them exact-safe.
Section~\ref{sec:kernels} covers the kernels that implement them.

\subsection{Bound and pruning rules}
...
intervals support a \emph{token-level} test inside one document. Write
$U_{ij} = P_{ij} + R_{ij}$ and $L_{ij} = P_{ij} - R_{ij}$, and for each
query token $i$ let $L_i = \max_{j \in \mathrm{Live}} L_{ij}$ over the
document's still-live tokens: a token $j$ is dropped once $U_{ij} < L_i$ for
every query token $i$.

\subsection{Exact safety}
\label{sec:bound:safety}

The document test and the token test together preserve exactness.
...
The unit-norm precondition deserves emphasis: it is a \emph{safety}
precondition, not a convenience. If a stored norm exceeds~1, the residual is
underestimated and exactness silently breaks, so our pipeline verifies norms
at export time.
...
\subsection{Transfer to a blocked kernel}
\label{sec:bound:transfer}

Two further lemmas transfer the proof from the per-document formulation to
the blocked kernel of Section~\ref{sec:kernels:bond}: a coarser (checkpointed,
...
Section~\ref{sec:bound:transfer}). At query time, an AVX-512 register tile
