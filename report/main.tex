...
The validity of every pruning claim depends on the strength of the dense
baseline it is measured against: a pruned kernel that beats a weak scan
demonstrates nothing about pruning. Because MaxSim is GEMM-shaped
(Section~\ref{sec:background}), the appropriate dense reference is not a
columnar scan but the matrix formulation of Equation~\ref{eq:maxsim}: with
query tokens stacked as $Q \in \mathbb{R}^{m \times D}$ and corpus tokens as
$X \in \mathbb{R}^{N \times D}$, all pair similarities are one product
$Q X^\top$, followed by a per-document segmented maximum. Executed through a
multithreaded Basic Linear Algebra Subprograms (BLAS) library, this
formulation outperforms a straightforward columnar scan by nearly an order
of magnitude on our hardware, and it is the baseline any MaxSim method must
be held to.

This formulation can itself be improved, because a general-purpose BLAS
performs three steps that MaxSim does not require. It packs the corpus
operand into cache-friendly tiles on every call, although the corpus is
static; it materializes the full $m \times N$ score matrix, although only
one maximum per document survives; and the segmented maximum then re-reads
that matrix from memory. Our kernel removes all three. The corpus is packed
once, at build time, into panel-major PDX blocks: panels of 16 document
tokens stored dimension-major, with documents padded to a panel multiple by
duplicating their last token (exact by the padding lemma of
Section~\ref{sec:bound:safety}). At query time, an AVX-512 register tile
accumulates all $m$ query tokens against one panel across the full dimension
range, and the per-document maximum is folded in registers as each
document's last panel completes, so the score matrix is never written. The
resulting kernel computes identical exact scores at roughly $3\times$ the
throughput of the 12-thread OpenBLAS formulation, and roughly $2\times$
single-threaded. It serves as the dense baseline for all pruning
experiments, and constitutes a contribution independent of pruning: the PDX
layout pays off for MaxSim through packing amortization and a fused
reduction, a mechanism distinct from the memory-bound argument of the
$m = 1$ literature.
...
checkpoint evaluates the document upper bound $\mathrm{UB}_d$ of
Equation~\ref{eq:ubd} against a shared, rising threshold $\tau$. If the document survives all checkpoints, the final
...
Section~\ref{sec:bound:rules} with lane masking, and a \emph{cheap-bound}
arm that relaxes the document-side residual to 1, mirroring the query-only
bound of the original BOND paper. The checkpoint set $C$ and the threshold
policy (self-seeded, externally seeded, or an oracle threshold for
accounting) are parameters throughout.

The cheap-bound arm deserves a note in advance. For Euclidean search, the
original BOND study concluded that the cheap bound wins at wall-clock: its
looseness costs little pruning and its bookkeeping is far cheaper. For
MaxSim we find the opposite (Section~\ref{sec:results}): the cheap bound
prunes nothing at any checkpoint where pruning is possible at all, so the
tight bound is the better choice despite its higher evaluation cost. The
tightness a Euclidean partial distance provides for free must, for an inner
product, be bought with the full Cauchy--Schwarz residual.
