...
A pruned kernel that beats a weak scan demonstrates nothing about pruning,
so the dense baseline must be as strong as possible. Because MaxSim is
GEMM-shaped (Section~\ref{sec:background}), that baseline is the matrix
formulation of Equation~\ref{eq:maxsim}: stacking query tokens as
$Q \in \mathbb{R}^{m \times D}$ and corpus tokens as
$X \in \mathbb{R}^{N \times D}$, one product $Q X^\top$ followed by a
per-document maximum. Through a multithreaded Basic Linear Algebra
Subprograms (BLAS) library, this outperforms a columnar scan by nearly an
order of magnitude on our hardware.
