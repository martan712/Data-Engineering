...
to an audited artifact. The key components are as follows:
...
\begin{itemize}
  ...
  \item \textbf{Controlled runners.} \texttt{experiments/pipeline/08}
    ...
    release runs.
  \item \textbf{Figures and tables.} Figures
    \ref{fig:quality-latency} to \ref{fig:bond} are generated directly from the
    release JSON files by \texttt{experiments/pipeline/09}, \texttt{11},
    \texttt{12}, and \texttt{14}; \texttt{tests/test\_report\_consistency.py}
    checks the readable LaTeX tables against the same JSON values.
\end{itemize}
