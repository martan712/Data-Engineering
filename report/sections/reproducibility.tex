...
\begin{itemize}
  \item \textbf{Release artifacts.} The five JSON files under
    \texttt{results/final/} are the authoritative evidence. Each records the
    Git commit (\texttt{0cc6145}, \texttt{dirty=false}), SHA-256 hashes of the
  ...
  \item \textbf{Pinned environments.} Python dependency manifests are pinned;
    ...
    Key package versions recorded in the artifacts include NumPy 2.4.6 and
    faiss-cpu 1.14.2 on Python 3.14.4.
  ...
  \item \textbf{Controlled runners.} \texttt{experiments/pipeline/08} (IVF)
    and \texttt{experiments/pipeline/10} (BOND) implement the protocol of
  ...
  \item \textbf{Figures from artifacts.} Figures
    ...
    release JSON files by \texttt{experiments/pipeline/09}, \texttt{11}, and
    \texttt{12}, without hard-coded headline values.
\end{itemize}
