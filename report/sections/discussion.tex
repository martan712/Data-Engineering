...
\paragraph{PRQ (BOND acceleration).} Exact-safe BOND dimension pruning did 
not accelerate exact ColBERT MaxSim over the project's compiled exhaustive 
kernel. On raw 128-dimensional embeddings, the kernel retained 95.47\% of 
exhaustive component products on SciFact and 94.16\% on NFCorpus, while running 
10.76 times and 9.84 times slower than the precision-matched exhaustive arm of 
the same interleaved run. This result is a controlled measurement with identical 
inputs, precision, thread budgets, and timing boundaries in both arms, and it 
replicates across two datasets.

\paragraph{RQ1 (bound behavior).} The residual Cauchy-Schwarz bound stays
...
earlier and removes 31 to 38\% of products, so dimension order is genuinely part
of the problem. However, the persisting 8.38 to 8.51$\times$ slowdown shows that
...
\paragraph{RQ2 (flat token index as candidate generator).} With $L = 100$ and
...
\paragraph{RQ3 (PDX-IVF versus FAISS-IVF).} The two engines produced
...
\paragraph{RQ4 (controlled PLAID comparison).} The two PLAID configurations
