...
All results in \cref{sec:results-ivf,sec:results-bond,sec:results-pca} were
...
\begin{itemize}
  ...
  \item \textbf{Matched work and output contract.} Every arm must emit ranked
    document identifiers under the same exact MaxSim definition; approximate
    arms are compared at matched rerank budgets.
  ...
  \item \textbf{Provenance.} Every result JSON records the Git commit and
    ...
    manifest accepts only artifacts from clean commit \texttt{0cc6145}.
\end{itemize}
...
\SI{2.93}{\second} SciFact exhaustive median) must not be compared with
float64 BOND-comparison latencies (e.g., the \SI{4.27}{\second} SciFact
