...
| 3 Mechanism experiments | `experiments/stage3_mechanism/` | In progress. e01+e02 (RQ1) done on scifact+nfcorpus; e03 run on scifact (negative at C={32,64}); e07 run on scifact/nfcorpus/arguana (reorder cost negligible); e08 run on all 4 datasets — LATE checkpoints (C={96}/{112}) prune 88–98% exact-safe and beat dense on 3/4 (RQ3 split verdict); e09 run on all 4 (both bounds lose at C={32,64}; bound decision deferred to R12). Remaining: e03/e04 other datasets, e07 scidocs, e05/e06 runs, R12 late-checkpoint follow-ups (see plan doc "Status And Research Plan") |
