...
| 5 CoRECT IR evaluation | `experiments/stage5_corect/`, `src/bondmaxsim/eval/` | In scope, next up (R9 rescoped back in 2026-07-09 as the paper's final section, after a brief 2026-07-08 deferral). Evaluate the best Stage 3–4 methods in the proper IR framework: qrels metrics (nDCG@10, recall@100, MRR@10, recall_vs_exact@10) + CoRECT RC via a ColBERT/MaxSim adapter over `extern/CoRECT`; arms = dense_fused, exact-safe BOND (TIGHT/natural/C={112}/self_bound), partitioned scan at 2–3 e03 operating points, tuned faiss_ivf/plaid at matched budgets; same interleaved one-stack controls as Stage 4. See the R9 plan in the plan doc |
