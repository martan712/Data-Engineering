...
| 3 Mechanism experiments | `experiments/stage3_mechanism/` | In progress. e01–e04 + e06 done on all 4 datasets (e03/e04: exact-safe BOND at default C={32,64} never beats dense; e06: policy choice moves cells only a few points); e05 done on scifact+nfcorpus (no G2 point at default C); e07 done on scifact/nfcorpus/arguana (reorder cost negligible; scidocs deferred — memory issue); e08 done on all 4 — LATE checkpoints (C={96}/{112}) prune 88–98% exact-safe and beat dense on 3/4 (RQ3 split verdict); e09 done on all 4 (both bounds lose at C={32,64}; bound decision deferred to R12). Remaining: R12 late-checkpoint follow-ups, e05 remainder at the winning config (see plan doc "Status And Research Plan") |
