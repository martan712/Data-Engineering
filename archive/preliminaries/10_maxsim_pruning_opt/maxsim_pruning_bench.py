"""
Experiment 10: OPTIMIZED MaxSim pruning (exp 09 kernel + dense-warmup→positional + token-major).
Same algorithm/bounds/recall as exp 09; rewritten for wall-clock vs the brute-force baseline.
...
def plot(results):
    ...
    fig.suptitle(
        "Experiment 10 — OPTIMIZED MaxSim pruning (dense-warmup→positional, token-major): BOND vs ADSampling vs natural\n"
...
def amortization_experiment():
    print("\n" + "=" * 80)
    print("Exp 10b — amortization sweep over query-token count m, SAFE mode (recall 1.0), per dataset")
    ...
    plot_amortization_grid(out, "maxsim_amortization.png",
        "Experiment 10b — BOND vs natural vs ADA over query-token count m, SAFE mode (recall 1.0)  "
        "— one row per dataset\n"
...
def amortization_recall_experiment():
    print("\n" + "=" * 80)
    print(f"Exp 10c — same sweep, each arm tuned per-m to ~{int(TARGET_REC*100)}% recall (APPROXIMATE), per dataset")
    ...
    plot_amortization_grid(out, "maxsim_amortization_recall.png",
        f"Experiment 10c — BOND vs natural vs ADA over m, tuned to ~{int(TARGET_REC*100)}% recall "
...
def main():
    print(f"Experiment 10: MaxSim pruning — D={D}, K={K}, ≤{MAX_DOCS} docs, ≤{MAX_QUERIES} queries/dataset\n")
    loaded = {}
    for ds in DATASETS:
        d, q = load_dataset(ds)
        loaded[ds] = (d, q)
...
def amort():
    """Exp 10b + 9c only (the amortization sweeps) — no 3-dataset main analysis."""
    print("Experiment 10 (amortization sweeps 10b + 10c) — per-dataset, over query-token count m\n")
    amortization_experiment()          # 10b — safe mode (recall 1.0)
    amortization_recall_experiment()   # 10c — tuned to ~TARGET_REC recall
