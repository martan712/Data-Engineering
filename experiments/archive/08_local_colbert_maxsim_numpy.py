"""Exact NumPy ColBERT MaxSim reference for the local corpus."""
...
OUTPUT_PATH = RESULTS_DIR / "local_colbert_maxsim_numpy.json"


def maxsim_scores(query: np.ndarray, documents: list[np.ndarray]) -> np.ndarray:
    """Score one query against all normalized document token matrices."""
    scores = []
...
def main() -> None:
    ...
    print("Local exact ColBERT MaxSim")
    ...
    for result in query_results:
        print(f"{result['query_id']} top5: {result['top10'][:5]}")
