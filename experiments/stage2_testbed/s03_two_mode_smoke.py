...
bondmaxsim.testbed.runner.Runner on a handful of scifact queries — for the
per-document-oracle kernel AND the wide-block kernel — and write one
ResultRecord per kernel/mode.  Confirms the accounting/throughput split holds
...
Output naming:
  results/json/stage2_testbed_two_mode_smoke_<dataset>_<mode>.json        (oracle kernel)
  results/json/stage2_testbed_two_mode_smoke_<dataset>_wide_<mode>.json   (wide kernel)
...
def run_two_modes(runner: Runner, cfg: RunConfig, tag: str, suffix: str) -> None:
    """Run accounting + throughput for one kernel config and write both JSONs."""
    # ---- Accounting mode ----
    acc_record = runner.accounting_mode(cfg)
    print(f"  [{tag}] accounting: recall={acc_record.recall_vs_exact_at_10:.3f}  "
    ...
    acc_path = RESULTS_JSON / f"stage2_testbed_two_mode_smoke_{DATASET}{suffix}_accounting.json"
    acc_record.to_json(acc_path)
    print(f"  JSON: {acc_path}")

    # ---- Throughput mode ----
    thr_record = runner.throughput_mode(cfg, n_repeats=3)
    print(f"  [{tag}] throughput: recall={thr_record.recall_vs_exact_at_10:.3f}  "
    ...
    thr_path = RESULTS_JSON / f"stage2_testbed_two_mode_smoke_{DATASET}{suffix}_throughput.json"
...
def main() -> None:
    ...
    # ---- Per-document oracle kernel (unchanged file naming) ----
    oracle_cfg = RunConfig(
        dataset=DATASET,
    ...
    run_two_modes(runner, oracle_cfg, tag="oracle", suffix="")
    ...
    wide_cfg = RunConfig(
        ...
        k=10,
        shrink=1.0,
        notes="s03 two-mode smoke test (wide-block kernel)",
    )
    run_two_modes(runner, wide_cfg, tag="wide", suffix="_wide")
